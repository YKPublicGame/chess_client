...
namespace ProtoBuf.ServiceModel
{
    ...
    public sealed class XmlProtoSerializer : XmlObjectSerializer
    {
        ...
        {
			if (model == null) throw new ArgumentNullException(model.GetType().Name);
			if (key < 0) throw new ArgumentOutOfRangeException(key.GetType().Name);
			if (type == null) throw new ArgumentOutOfRangeException(type.GetType().Name);
            this.model = model;
            this.key = key;
            this.isList = isList;
            this.type = type;
        ...
        {
			if (model == null) throw new ArgumentNullException(model.GetType().Name);
			if (type == null) throw new ArgumentNullException(type.GetType().Name);
        ...
        public XmlProtoSerializer(TypeModel model, Type type)
        {
            ...
            this.type = type;
            this.isEnum = Helpers.IsEnum(type);
			if (key < 0) throw new ArgumentOutOfRangeException(type.GetType().Name, "Type not recognised by the model: " + type.FullName);
        }
        ...
        {
			if (writer == null) throw new ArgumentNullException(writer.GetType().Name);
        ...
        {
			if (reader == null) throw new ArgumentNullException(reader.GetType().Name);
